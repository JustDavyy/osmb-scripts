...
public class Fight extends Task {
    ...
    public boolean execute() {
        ...
        if (foundCrab) {
            ...
            return script.submitHumanTask(this::fightMrCrabs, script.random(630000, 720000));
        } else {
    ...
    private boolean fightMrCrabs() {
        ...
        if (needToEat()) {
            ...
            script.submitHumanTask(waitCon, script.random(4000, 8000));
        }
        ...
        if (usePot) {
            ...
            if (needToPot()) {
                ...
                script.submitTask(() -> false, script.random(1200, 2000));
                if (!initiateAttack()) {
                    initiateAttack();
                }
                ...
                script.submitHumanTask(waitCon, script.random(5000, 10000));
            }
        }
        ...
        if (needToBoost()) {
            if (useDBAXE) {
                ...
                boolean succ = script.submitTask(() -> {
                    ItemGroupResult dbaxe = script.getWidgetManager().getInventory().search(Set.of(ItemID.DRAGON_BATTLEAXE));
                    if (dbaxe == null) return false;
                    return dbaxe.contains(ItemID.DRAGON_BATTLEAXE);
                }, script.random(3000, 5000));
        ...
        if (!crabActive) {
            ...
            if (!handleObject("Cave", "Crawl-through", getClosestCaveSpot(), getClosestCaveArea())) {
                ...
                if (script.submitHumanTask(() -> getClosestCrabArea() != currentArea || isDialogueOpen(), script.random(15000, 20000))) {
                    ...
                    script.log(getClass(), "Add additional humanized delay");
                    task = "Additional human delay";
                    script.submitHumanTask(() -> false, script.random(1, 4000));
                    if (findCrabNPC(true)) {
                        boolean succeeded = initiateAttack();
            ...
            if (chooseAttack) {
                ...
                for (int attempt = 1; attempt <= 3 && !ok; attempt++) {
                    ...
                    if (!ok) {
                        ...
                        script.submitHumanTask(() -> false, script.random(120, 420));
                    }
                }
    ...
    private boolean initiateAttack() {
        ...
        script.submitHumanTask(() -> false, script.random(2000, 5000));
    ...
    private boolean handleObject(String objectName, String objectAction, WorldPosition objectLocation, Area objectArea) {
        ...
        if (target.interact(objectAction)) {
            script.submitHumanTask(() -> false, script.random(3500, 5000));
            resetAntiAfkTimer();
            return true;
        }
        ...
        if (success) script.submitHumanTask(() -> false, script.random(3500, 5000));
        return success;
    }
    ...
    private boolean crabActive3s() {
       return  script.submitHumanTask(() -> findCrabNPC(false), script.random(3000, 4000));
    }
    ...
    private boolean waitCrabDieThenTraverseCave() {
        ...
        boolean died = script.submitHumanTask(() -> !crabActive3s(), script.random(12_000, 15_000));
        if (!died) {
        ...
        // Small settle delay
        script.submitHumanTask(() -> false, script.random(8000, 14000));
        return true;
    }
