...
public class XPTracking {
    ...
    private static final int SPRITE_TOTAL_ID = 222;

    private final ScriptCore core;
    private final SearchableImage totalSprite;
    ...
    public XPTracking(ScriptCore core) {
        ...
        SearchableImage totalFull = new SearchableImage(SPRITE_TOTAL_ID, core, new SingleThresholdComparator(15), ColorModel.RGB);
        this.totalSprite = totalFull.subImage(totalFull.width / 2, 0, totalFull.width / 2, totalFull.height);
    }
    ...
    private Integer getXpCounter() {
        ...
        boolean matchestotal = core.getImageAnalyzer().findLocation(bounds, totalSprite) != null;
        if (!matchestotal) {
            return null;
        }
