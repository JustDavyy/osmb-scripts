...
public class Setup extends Task {
    ...
    public boolean execute() {
        ...
        script.log(dTeleporter.class, "Opening magic tab");
        script.getWidgetManager().getTabManager().openTab(Tab.Type.SPELLBOOK);

        script.log(dTeleporter.class, "Closing chatbox (if open)");
        closeChatBox();
    ...
    public void closeChatBox() {
        ...
        if(chatboxComponent.isOpen()) {
            ...
            if(chatBoxTabBounds == null) {
                script.log(dTeleporter.class, "Chatbox bounds are null, cannot close Chatbox.");
                return;
            }
