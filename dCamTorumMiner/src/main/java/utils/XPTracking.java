...
public class XPTracking {
    ...
    public XPTracking(ScriptCore core) {
        ...
        SearchableImage miningFull = new SearchableImage(SPRITE_MINING_ID, core, new SingleThresholdComparator(15), ColorModel.RGB);
        this.miningSprite = miningFull.subImage(miningFull.width / 2, 0, miningFull.width / 2, miningFull.height);
    }
    ...
    private Integer getXpCounter() {
        ...
        boolean matchesmining = core.getImageAnalyzer().findLocation(bounds, miningSprite) != null;
        if (!matchesmining) {
            return null;
        }
