...
public class XPTracking {
    ...
    private static final int SPRITE_RANGED_ID = 200;

    private final ScriptCore core;
    private final SearchableImage rangedSprite;
    ...
    public XPTracking(ScriptCore core) {
        ...
        SearchableImage rangedFull = new SearchableImage(SPRITE_RANGED_ID, core, new SingleThresholdComparator(15), ColorModel.RGB);
        this.rangedSprite = rangedFull.subImage(rangedFull.width / 2, 0, rangedFull.width / 2, rangedFull.height);
    }
    ...
    private Integer getXpCounter() {
        ...
        boolean matchesranged = core.getImageAnalyzer().findLocation(bounds, rangedSprite) != null;
        if (!matchesranged) {
            return null;
        }
