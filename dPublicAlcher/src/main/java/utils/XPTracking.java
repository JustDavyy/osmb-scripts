...
public class XPTracking {
    ...
    private static final int SPRITE_MAGIC_ID = 202;

    private final ScriptCore core;
    private final SearchableImage magicSprite;
    ...
    public XPTracking(ScriptCore core) {
        ...
        SearchableImage magicFull = new SearchableImage(SPRITE_MAGIC_ID, core, new SingleThresholdComparator(15), ColorModel.RGB);
        this.magicSprite = magicFull.subImage(magicFull.width / 2, 0, magicFull.width / 2, magicFull.height);
    }
    ...
    private Integer getXpCounter() {
        ...
        boolean matchesmagic = core.getImageAnalyzer().findLocation(bounds, magicSprite) != null;
        if (!matchesmagic) {
            return null;
        }
