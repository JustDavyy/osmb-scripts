...
public class Tempoross extends Task {
    ...
    // Object stuff
    public static final String[] POOL_NAMES = {"Empty pool"};
    public static final String[] POOL_ACTIONS = {"big-search"};
    public static final Predicate<RSObject> poolQuery = gameObject -> {
        if (gameObject.getName() == null || gameObject.getActions() == null) return false;
        if (Arrays.stream(POOL_NAMES).noneMatch(name -> name.equalsIgnoreCase(gameObject.getName()))) return false;
        return Arrays.stream(gameObject.getActions()).anyMatch(action -> Arrays.stream(POOL_ACTIONS).anyMatch(poolAction -> poolAction.equalsIgnoreCase(action)))
                && gameObject.canReach();
    };

    // Areas/Pathing/Mapping
    private static final Area bankArea = new RectangleArea(3156, 2834, 2, 2, 0);
    private static final Area poolArea = new RectangleArea(3153, 2832, 1, 3, 0);
    private static final WorldPosition netNPCPos = new WorldPosition(3153, 2831, 0);

    // Chat history stuff
    private static final List<String> PREVIOUS_CHATBOX_LINES = new ArrayList<>();

    public Tempoross(Script script) {
        super(script);
    ...
    public boolean execute() {
        ...
        ItemGroupResult invSlots = script.getWidgetManager().getInventory().search(Set.of(ItemID.SMALL_FISHING_NET));
        if (invSlots == null) return false;
        if (!invSlots.isFull()) {

            // Check if we have a net, if not get it from the NPC
            AtomicReference<ItemGroupResult> inv = new AtomicReference<>(script.getWidgetManager().getInventory().search(Set.of(ItemID.SMALL_FISHING_NET)));
            boolean hasNet = inv.get() != null && inv.get().contains(ItemID.SMALL_FISHING_NET);

            if (!hasNet) {
                script.log(getClass(), "Getting net from NPC...");

                // Build a game-screen shape for the NPC tile (null/visibility safe)
                var projector = script.getSceneProjector();
                boolean tapped = false;
                if (projector != null) {
                    var cube = projector.getTileCube(netNPCPos, 35, true);
                    if (cube != null) {
                        var shape = cube.getResized(0.3);
                        if (shape != null) {
                            tapped = script.getFinger().tapGameScreen(shape, "take-net");
                        }
                    }
                }

                // Wait until we have the net
                ...
                boolean gotNet = script.submitHumanTask(condition, script.random(5000, 10000));

                if (gotNet) {
                    return true;
                }

                // Still no net → walk toward the pool area before bailing out
                script.log(getClass(), "No net detected after waiting. Walking toward the reward pool area before retry/stop.");

                // Try to find a pool object to use as a break condition (same guardianQuery you use elsewhere)
                RSObject pool = null;
                List<RSObject> objectsFound = script.getObjectManager().getObjects(poolQuery);
                if (objectsFound != null && !objectsFound.isEmpty()) {
                    pool = (RSObject) script.getUtils().getClosest(objectsFound);
                }

                WalkConfig config = new WalkConfig.Builder()
                        .breakCondition(pool != null ? pool::isInteractableOnScreen : null)
                        .enableRun(true)
                        .build();

                var targetPos = poolArea.getRandomPosition();
                if (targetPos != null) {
                    script.getWalker().walkTo(Objects.requireNonNull(targetPos), config);
                }
    ...
    private void waitUntilDoneLooting() {
        ...
        BooleanSupplier condition = () -> {
            ...
            if (type != null && type.equals(DialogueType.TAP_HERE_TO_CONTINUE)) {
                ...
                if (dialogueText.contains("anything here for you")) {
                    script.log(getClass(), "Detected phrase: There doesn't seem to be anything here for you. Maybe you could work with the Spirit Anglers...");
                    script.log(getClass(), "We're out of searches, marking script to stop!");
                    needToStop = true;
                    return true;
                }

                if (dialogueText.contains("inventory space")) {
        ...
        script.submitHumanTask(condition, script.random(150000, 250000));
        ...
        if (script.random(10) < 3) {
            ...
            script.submitHumanTask(() -> false, script.random(150, 500));
        }
    }
    ...
    private boolean startSearch() {
        script.log(getClass(), "Get Reward pool object...");

        final int maxAttempts = 3;
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            List<RSObject> objectsFound = script.getObjectManager().getObjects(poolQuery);
            if (objectsFound.isEmpty()) {
                script.log(getClass(), "Reward pool object not found (attempt " + attempt + "/" + maxAttempts + ").");
                continue;
            }
            ...
            if (!walked) {
                ...
                script.submitHumanTask(() -> false, script.random(300, 700));
                continue;
            }

            task = "Search reward pool";
            if (guardian.interact("big-search")) {
                return true;
            }

            script.log(getClass(), "Failed to interact with Reward pool (attempt " + attempt + "/" + maxAttempts + "). Retrying...");
            script.submitHumanTask(() -> false, script.random(400, 900));
        }
    ...
    private void processNewChatboxMessages(List<String> newLines) {
        for (String message : newLines) {
            ...
            if (lower.contains("funny feeling")) {
                ...
                totalGained.merge(ItemID.TINY_TEMPOR, 1, Integer::sum);
                script.log(getClass(), "Tiny Tempor drop detected via chat message!");
                return;
            }

            if (lower.contains("more rewards")) {
