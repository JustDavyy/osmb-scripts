...
public class XPTracking {
    ...
    public XPTracking(ScriptCore core) {
        ...
        SearchableImage craftingFull = new SearchableImage(SPRITE_CRAFTING_ID, core, new SingleThresholdComparator(15), ColorModel.RGB);
        this.craftingSprite = craftingFull.subImage(craftingFull.width / 2, 0, craftingFull.width / 2, craftingFull.height);
    }
    ...
    private Integer getXpCounter() {
        ...
        boolean matchescrafting = core.getImageAnalyzer().findLocation(bounds, craftingSprite) != null;
        if (!matchescrafting) {
            return null;
        }
