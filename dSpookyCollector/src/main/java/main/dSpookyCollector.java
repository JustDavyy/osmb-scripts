...
public class dSpookyCollector extends Script {
    ...
    public dSpookyCollector(Object scriptCore) {
        super(scriptCore);
    }

    @Override
    public int[] regionsToPrioritise() {
        return new int[]{12597, 12835};
    }

    @Override
    public void onStart() {
        log("INFO", "Starting dSpookyCollector v" + scriptVersion);
        checkForUpdates();
        ...
        itemToCollect = ui.getSelectedItem();

        tasks = new ArrayList<>();
        tasks.add(new Setup(this));
        tasks.add(new Collector(this));
    }
    ...
    public void onPaint(Canvas c) {
        ...
        int itemsPerHour = (int) ((itemsCollected * 3600000L) / elapsed);
        ...
        int y = 40;
        c.fillRect(5, y, 250, 100, Color.BLACK.getRGB(), 1);
        c.drawRect(5, y, 250, 100, Color.BLACK.getRGB());

        c.drawText("Items collected: " + f.format(itemsCollected), 10, y += 20, Color.WHITE.getRGB(), ARIEL);
        c.drawText("Items/hr: " + f.format(itemsPerHour), 10, y += 20, Color.WHITE.getRGB(), ARIEL);
        y += 10;
    ...
    private void sendWebhook() {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            json.append("{\"embeds\":[{")
                    .append("\"title\":\"📊 dSpookyCollector Stats - ")
                    .append(webhookShowUser && user != null ? escapeJson(user) : "anonymous").append("\",")
                    .append("\"color\":15844367,");

            if (webhookShowStats) {
                json.append("\"fields\":[")
                        .append("{\"name\":\"Items collected\",\"value\":\"").append(f.format(itemsCollected)).append("\",\"inline\":true},")
                        .append("{\"name\":\"Items/hr\",\"value\":\"").append(f.format(itemsPerHour)).append("\",\"inline\":true},")
    ...
    private void checkForUpdates() {
        String latest = getLatestVersion("https://raw.githubusercontent.com/JustDavyy/osmb-scripts/main/dSpookyCollector/src/main/java/main/dSpookyCollector.java");
        if (latest == null) {
        ...
        if (compareVersions(scriptVersion, latest) < 0) {
            ...
            try {
                ...
                File[] old = dir.listFiles((d, n) -> n.equals("dSpookyCollector.jar") || n.startsWith("dSpookyCollector-"));
                if (old != null) for (File f : old) if (f.delete()) log("UPDATE", "🗑 Deleted old: " + f.getName());

                File out = new File(dir, "dSpookyCollector-" + latest + ".jar");
                URL url = new URL("https://raw.githubusercontent.com/JustDavyy/osmb-scripts/main/dSpookyCollector/jar/dSpookyCollector.jar");
